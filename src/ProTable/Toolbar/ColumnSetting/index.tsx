...
const ColumnSetting: React.FC<ColumnSettingProps> = () => {
  ...
  const options: any = columns.map((item) => ({
    label: item.title,
    ...
    <div className="iLab-pro-table-toolbar-column-setting">
      <div className="iLab-pro-table-toolbar-column-setting-header">
        ...
        <div>
          <Typography.Link
            onClick={() => {
              const defaultChecked = columns.filter((item) => !(item.disabled && !(checkFlip ? item.setting : !item.setting)));
              localStorage.setItem(
                `${window.location.pathname}-${id}-Col`,
                defaultChecked.map((item) => item.dataIndex).join(),
              );
              setSelectedDataIndex(defaultChecked.map((item) => item.dataIndex));
            }}
          >
          ...
          <Typography.Link
            style={{ marginLeft: 6 }}
            onClick={() => {
              const filterColumn = columns.filter((it) => (checkFlip ? it.setting : !it.setting));
              localStorage.setItem(
                `${window.location.pathname}-${id}-Col`,
