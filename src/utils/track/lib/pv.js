import { emit, pageId, traceThirdPartyEvent } from './base';
...
function tracePageView(option = {}) {
  const { url = window.location.href, referer = oldURL, actions = '', params, type } = option;
  let action = actions;
  ...
  setTimeout(() => {
    if (type === 'ym') {
      traceThirdPartyEvent('_czc', '_trackEvent', '页面', option.url);
    } else {
      emit({
        eventType: 'pv',
        eventId: pageId,
...
function init(options = {}) {
  ...
  if (window.history.pushState) {
    ...
    window.history.pushState = (data, title, url) => {
      ...
      tracePageView({ actions: 'navigation', type });
      return result;
    };
  ...
  window.addEventListener('hashchange', () => {
    if (pvHashtag && !lastIsPop) tracePageView({ type });
    lastIsPop = false;
  });
}
