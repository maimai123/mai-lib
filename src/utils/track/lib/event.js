  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      const target = path.find(
        (
          el, // 检查被点击的元素以及其父级元素是否有这些属性(从内到外,只会取第一个检查到的)
        ) =>
          el.hasAttribute &&
...
function extractDataByPath(list = []) {
  ...
  if (container) {
    if (
      container.getAttribute('data-warden-event-id') ||
...
function extractParamsByPath(list = []) {
  ...
  try {
    ...
    list.forEach((el, index) => {
      const attributes =
        (el && el.attributes && Array.from(el.attributes)) || [];
 ...
 * @returns
 */
function traceCustomEvent(params = {}, eventId, title) {
  emit({
    eventId,
    title,
    params,
